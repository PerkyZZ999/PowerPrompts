...
const ConfigSchema = z.object({
  ...
  openrouterMaxPromptPrice: z.coerce.number().default(0.20), // $0.20 per 1M tokens (allows Google Vertex: $0.15)
  openrouterMaxCompletionPrice: z.coerce.number().default(1.00), // $1.00 per 1M tokens (allows Google Vertex: $0.60)
...
function loadConfig(): Config {
  try {
    const config = ConfigSchema.parse({
      ...
      openrouterMaxCompletionPrice: process.env['OPENROUTER_MAX_COMPLETION_PRICE'],
...
function generateModelDescription(modelId: string): string {
  ...
  // GPT models
  if (modelName.includes('gpt-oss-120b')) return 'Fast, open-source model with high throughput (Google Vertex: 547.8 TPS)';
  if (modelName.includes('gpt-5')) return 'Most capable model for complex tasks';
  if (modelName === 'gpt-4o') return 'High-intelligence flagship model';
  if (modelName === 'gpt-4o-mini') return 'Affordable and intelligent small model';
  if (modelName.includes('gpt-4-turbo')) return 'Previous high-intelligence model';
  if (modelName.includes('gpt-3.5')) return 'Fast and efficient model';
  
  // Reasoning models
  if (modelName === 'o1') return 'Reasoning model designed to solve hard problems';
  ...
  // Claude models
  if (modelName.includes('claude-3.5-sonnet')) return 'Most intelligent Claude model';
  if (modelName.includes('claude-3-opus')) return 'Powerful model for complex tasks';
  if (modelName.includes('claude-3-sonnet')) return 'Balanced intelligence and speed';
