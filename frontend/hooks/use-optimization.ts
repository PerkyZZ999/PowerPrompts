...
export function useOptimization() {
  ...
  const startOptimization = useCallback(async () => {
    ...
    console.log(
      "  - prompt:",
      store.prompt ? `"${store.prompt.substring(0, 50)}..."` : "(empty)",
    );
      ...
      switch (event.type) {
        case "optimization_start":
          ...
          toast.info(
            ...
            `${event.data.total_iterations} iteration(s)`,
            2000,
          );
          break;
        ...
        case "iteration_start":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Running tests...`,
          );
          toast.info(
        ...
        case "executing_tests":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Executing ${event.data.count} tests...`,
          );
        ...
        case "test_progress":
          ...
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Test ${event.data.current}/${event.data.total}`,
          );
          break;
        ...
        case "applying_technique":
          ...
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Applying ${event.data.technique}...`,
          );
          break;

        case "evaluating_metrics":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Evaluating metrics...`,
          );
        ...
        case "metrics_calculated":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Metrics calculated`,
          );
          toast.info(
            `Metrics Calculated`,
            `Score: ${event.data.metrics.aggregate_score.toFixed(1)}`,
            2000,
          );
          break;
        ...
        case "applying_rsip":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Applying RSIP...`,
          );
          break;

        case "prompt_improved":
          store.setCurrentStep(
            `Iteration ${event.data.iteration}: Prompt improved via RSIP`,
          );
          toast.success(
            "Prompt Improved",
            `RSIP applied for iteration ${event.data.iteration + 1}`,
            2000,
          );
          break;
        ...
        case "optimization_complete":
          const completedResult: OptimizationComplete = {
            ...
            all_iterations: event.data.all_versions.map(
              (v: any, i: number) => ({
