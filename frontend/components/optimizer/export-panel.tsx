...
import { Download, FileJson, FileText, FileCode, CheckCircle2 } from "lucide-react";
import { useState } from "react";
import { toast } from "@/stores/toast-store";

export function ExportPanel() {
  const completedResult = useOptimizationStore((state) => state.completedResult);
  ...
  const exportAsJSON = () => {
    ...
    showSuccess("JSON");
    toast.success("Exported as JSON", "Your optimization results have been downloaded", 3000);
  };
...
${iterations.map(it => `### Iteration ${it.iteration}
...
`;
    ...
    toast.success("Exported as Markdown", "Your formatted report has been downloaded", 3000);
  };
...
`;
    ...
    showSuccess("Text");
    toast.success("Exported as Text", "Your plain text report has been downloaded", 3000);
  };
  ...
  return (
    <Card variant="glass">
      ...
      <CardContent className="space-y-4">
        {/* Export Description */}
        <p className="text-sm text-zinc-400">
          Download your optimization results in various formats for documentation, sharing, or integration.
        </p>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-zinc-200">Export as JSON</h4>
                  <p className="text-xs text-zinc-500">Complete data with all metrics and details</p>
                </div>
              </div>
              {exportedFormat === "JSON" && (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-zinc-200">Export as Markdown</h4>
                  <p className="text-xs text-zinc-500">Formatted report for documentation</p>
                </div>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-zinc-200">Export as Plain Text</h4>
                  <p className="text-xs text-zinc-500">Simple text file for easy reading</p>
                </div>
              </div>
              {exportedFormat === "Text" && (
                <CheckCircle2 className="w-5 h-5 text-primary animate-in zoom-in duration-200" />
        ...
        <div className="glass-elevated rounded-lg p-3 space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-primary font-mono font-semibold">{iterations.length}</span>
          </div>
          <div className="flex items-center justify-between text-xs">
            <span className="text-zinc-500">Best version:</span>
            <span className="text-primary font-mono font-semibold">
              {completedResult?.best_version || iterations[iterations.length - 1]?.version_id || "N/A"}
            </span>
          </div>
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-primary font-mono font-semibold">
              {completedResult?.final_metrics?.aggregate.toFixed(2) || 
               iterations[iterations.length - 1]?.metrics.aggregate.toFixed(2) || "N/A"}
            </span>
          </div>
        </div>
